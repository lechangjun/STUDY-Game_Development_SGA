...
Background::Background(wstring shaderFile)
{
	...
	textureFile = L"../_Textures/bgPuzzle2.png";
	Puzzle = new Sprite(textureFile, shaderFile );
}
...
{
	/*cloud[0]->Position(350, 620);
	cloud[0]->Render();

	cloud[0]->Position(70, 500);
	cloud[0]->Render();

	cloud[1]->Position(700, 700);
	cloud[1]->Render();
	...
	Puzzle->Position(Width / 2, Height / 2);
	Puzzle->Render();
